...
As we have seen in \RefSec{sec:pipeline}, the big majority of the work
...
it internally already has the Scala form. As \RefCode{lst:clstranslation}
...
called on. For example, \RefCode{lst:equals-scala} shows a call to
...
In \RefSec{sec:pipeline} we already saw how our specification attributes
on functions are desugared by the procedural macro of
\passthrough{\lstinline!libstainless!} into an annotated closure nested
inside the function (\RefCode{lst:example2}). The extraction phase
